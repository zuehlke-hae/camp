 ...
 *
 * @see SpringLayout
 * @see SpringConstraints.SpringConstraints
 *
 * @author      Philip Milne
 * @since       1.4
 */
public abstract class Spring {
     ...
     */
    public abstract double getPreferredValue();
    ...
    /*pp*/ static abstract class AbstractSpring extends Spring {
        protected double size = UNSET;

        public double getValue() {
            return size != UNSET ? size : getPreferredValue();
        }

        public final void setValue(double size) {
            if (this.size == size) {
                return;
            }
            if (size == UNSET) {
    ...
    public static double computeContentBias(Node node){
    	return node.getContentBias() == null ? -1 : 0;
    }
   ...
   /*pp*/ static abstract class SpringMap extends Spring {
       ...
       protected abstract double map(double i);

       protected abstract double inv(double i);
...
// cache values that have already been calculated.
     ...
     * @see Spring
     */
     public static Spring constant(double pref) {
    ...
    */
     ...
     * @since 1.5
     */
    public static Spring height(Node c) {
        checkArg(c);
